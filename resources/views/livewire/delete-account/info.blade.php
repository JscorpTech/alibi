...
<div>
    <div
        ...
        <h3 class="font-bold text-2xl text-black mb-2">{{ __("Delete Account?") }}</h3>
        <p>{{ __("You'll permanently loose your") }}</p>
        <ul class="list-disc inline-block text-center pt-2 ml-2">
            {!! strip_tags(Settings::get("delete:account")) !!}
        </ul>
        <div class="flex pt-8">
            <a
                href="/"
            ...
            <a href="{{ route("delete-account:phone") }}" class="w-1/2 transition ml-1 bg-black border border-black text-white hover:bg-red-700 hover:border-red-700 py-2 px-4 rounded font-medium">
                    {{ __("Delete") }}
            </a>
        </div>
    </div>
